...
def extract_annotations(closed_ended_response):
    ...
    for match in matches:
        ...
        annotation_key = (person, text.strip(), tuple(labels_list))
...
def highlight_text_in_pdf(user_input, annotations, output_pdf_filename):
    ...
    for ann in annotations:
        start_idx = user_input.find(ann['text'])
        if start_idx == -1 or (start_idx, ann['text']) in added_annotations:
            continue
        added_annotations.add((start_idx, ann['text']))

        label = ", ".join(ann['labels'])
        text_width = can.stringWidth(ann['text'], "Helvetica", 12)
        x_position = 1 * inch
        y_position = 10 * inch - (start_idx / len(user_input)) * 9 * inch
...
async def recogniser(do_open_ended_analysis=None, do_closed_ended_analysis=None, extract_message_indexes=None, extract_line_numbers=None, argv=None):
    ...
    if render_output:
        ...
        highlights_html, annotations = render_highlights_uncached(user_input, closed_ended_response)

...
letters_regex = regex.compile(
    r'\p{L}')  # matches unicode letters only, not digits   # regex has better Unicode support than re
...
model = AutoModelForCausalLM.from_pretrained(mistral_model_path)

def run_mistral(model, tokenizer, user_input, max_new_tokens=50, temperature=0.0):
...
def get_config():
    ...
    mistral_model_path = remove_quotes(config.get("MER", "MistralModelPath", fallback="mistralai/Mistral-7B-Instruct-v0.3")).strip()  # Ensure correct fallback and key name
    mistral_timeout = int(remove_quotes(config.get("MER", "MistralTimeoutInSeconds", fallback="600")).strip())
    ...
    treat_entire_text_as_one_person = strtobool(
        remove_quotes(config.get("MER", "TreatEntireTextAsOnePerson", fallback="false")))  # TODO
    ...
    named_entity_recognition_model = remove_quotes(
        config.get("MER", "NamedEntityRecognitionModel", fallback="en_core_web_sm")).strip()
    ...
    ignore_incorrectly_assigned_citations = strtobool(
        remove_quotes(config.get("MER", "IgnoreIncorrectlyAssignedCitations", fallback="false")))
    allow_multiple_citations_per_message = strtobool(
        remove_quotes(config.get("MER", "AllowMultipleCitationsPerMessage", fallback="true")))
    ...
    default_label_treshold_sample_percent = float(
        remove_percent(remove_quotes(config.get("MER", "DefaultLabelThresholdSamplePercent", fallback="50"))))
...
def remove_comments(text):
    text = re.sub(r"(^|[\r\n]+)\s*#[^\r\n]*", r"\1", text)  # keep newlines before the comment to preserve line indexing
    return text
...
def anonymise_uncached(user_input, anonymise_names, anonymise_numbers, ner_model):
    ...
    for phase in range(0, 2):  # two phases: 1) counting unique entities, 2) replacing them
        for word in entities.ents:
            ...
            text_normalised = re.sub(r"\s+", " ", text_original)  # normalise the dictionary keys
            ...
            if label == "PERSON":
                ...
                replacement = ("Number" if anonymise_numbers and len(text_normalised) > 2 and re.search(r"(\d|\s)", text_normalised) else None)
            else:
                replacement = None
...
async def anonymise(config, user_input, anonymise_names, anonymise_numbers, ner_model, enable_cache=True):
    user_input = re.sub(r"[^\S\r\n]+", " ", user_input)  # replace all repeating whitespace which is not newline with a single space

    encrypt_cache_data = config["encrypt_cache_data"]

    if encrypt_cache_data:
        result = await async_cached_encrypted(1 if enable_cache else None, anonymise_uncached, user_input,
                                              anonymise_names, anonymise_numbers, ner_model)
    else:
        result = await async_cached(1 if enable_cache else None, anonymise_uncached, user_input, anonymise_names,
                                    anonymise_numbers, ner_model)

    return result

def split_text_into_chunks_worker(paragraphs, separator, max_tokens_per_chunk):
    chunks = []
    ...
    for paragraph in paragraphs:
        ...
        if current_chunk_token_count > 0:
            if current_chunk_token_count + paragraph_token_count <= max_tokens_per_chunk:
                current_chunk_token_count += paragraph_token_count
                current_chunk.append(separator)  # keep original separators that were present in text
...
def split_text_into_chunks(paragraphs, separator, max_tokens_per_chunk):
    chunks = split_text_into_chunks_worker(paragraphs, separator, max_tokens_per_chunk)
    chunks = ["".join(chunk_paragraphs) for (chunk_paragraphs, chunk_token_count) in chunks]
    return chunks


def recogniser_process_chunk(user_input, config, instructions, tokenizer, model, do_open_ended_analysis=None,
                               do_closed_ended_analysis=None, extract_message_indexes=None,
                               extract_line_numbers=None, sample_index=0):
    mistral_model_path = config["mistral_model_path"]  # Changed from "mistral_model" to "mistral_model_path"
    ...
    if do_open_ended_analysis:
        ...
        user_input_string = user_input if isinstance(user_input, str) else messages
        open_ended_response = run_mistral(model, tokenizer, user_input_string)
    else:
    ...
    if do_closed_ended_analysis:
        ...
        user_input_string = user_input if isinstance(user_input, str) else messages
        closed_ended_response = run_mistral(model, tokenizer, user_input_string)
        ...
        if extract_message_indexes:
            ...
            user_input_string = user_input if isinstance(user_input, str) else messages
            names_of_participants_response = run_mistral(model, tokenizer, user_input_string)
        ...
        for person in detected_persons:
            ...
            for re_match in re_matches:
                ...
                start_char = re_match.start(1) + num_stripped_leading_chars - 1
        ...
        totals = defaultdict(lambda: OrderedDict([(label, 0) for label in
                                                  labels_list]))
        expression_dicts = []
        ...
        for tuple_index, expressions_tuple in enumerate(expressions_tuples):
            ...
            for person2 in detected_persons:
                ...
                match = rapidfuzz.process.extractOne(citation, curr_person_messages,
                ...
                if (
                        ...
                        or (similarity == nearest_message_similarity
                            and nearest_message_is_partial_match
                            and not similarity_is_partial_match)
                        or (similarity == nearest_message_similarity
                            and similarity_is_partial_match == nearest_message_is_partial_match
                            and person2 == person)
                ):
            ...
            if allow_multiple_citations_per_message:
                ...
                try:
                    ...
                    with time_limit(outer_time_limit, msg="find_near_matches outer") as time_limit_handler:
                        ...
                        for max_l_dist in range(0, min(len(citation), len(nearest_message))):
                            matches = find_near_matches(citation, nearest_message, max_l_dist=max_l_dist)

                            if len(matches) > 0:
                                time_limit_handler.disable_time_limit()
                            ...
                            if time_limit_handler.timeout_pending:
                                raise TimeoutError(
                                    "Timed out for operation '{0}' after {1} seconds".format(time_limit_handler.msg,
                                                                                             time_limit_handler.seconds))
                except TimeoutError:
                    safeprint(
                        f"Encountered a time limit during detection of citation location. tuple_index={tuple_index} max_l_dist={max_l_dist}. Skipping citation \"{citation}\". Is a similar line formatted properly in the input file?")
                    continue
            ...
            if extract_message_indexes:
                ...
                entry.update({"message_index": overall_message_index})

            if extract_line_numbers:
                line_number = bisect_right(line_start_char_positions, start_char)
                entry.update({"line_number": line_number + 1})
    ...
    result = (expression_dicts, totals, closed_ended_response, open_ended_response, num_messages, num_lines, user_input_len)
    return result

async def recogniser(do_open_ended_analysis=None, do_closed_ended_analysis=None, extract_message_indexes=None,
                     extract_line_numbers=None, argv=None):
    ...
    if input_filename:
        ...
        input_filename = "test_input6.txt"
    ...
    user_input = separator.join(paragraphs)

    max_tokens_per_chunk = 500  # Adjust as necessary

    with Timer("Splitting text into chunks with balanced lengths"):
        chunks = split_text_into_chunks(paragraphs, separator, max_tokens_per_chunk)
    ...
    for sample_index in range(0, sample_count):
        ...
        for index, chunk_text in enumerate(chunks):
            with Timer(f"Analysing chunk {(index + 1)} / {len(chunks)} of sample {(sample_index + 1)} / {sample_count}"):
                chunk_analysis_result = await recogniser_process_chunk(chunk_text, config, instructions, tokenizer, model,
                                                                       do_open_ended_analysis if sample_index == 0 else False,
                                                                       do_closed_ended_analysis,
                                                                       extract_message_indexes, extract_line_numbers, sample_index)
